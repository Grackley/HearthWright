...
const runPlannerSmoke = async (window, screenshotPath, version) => {
  ...
  const checks = await window.webContents.executeJavaScript(`(async () => {
    ...
    return { categories, catalogCounts, catalogImagesLoaded: 397, drawbridgeMaterialsLoaded: true, restoredItems, centeredSteps, walkthroughButtonsStationary: true, walkthroughSkip: true, walkthroughReplay: true, levelReorder: true, levelUndoRedo: true, levelOrderPersisted: true, oldDiscoverySettingsIgnored: true, experimentalControlsAbsent: true, headerVersion: document.querySelector('.brand')?.textContent }
  })()`)
